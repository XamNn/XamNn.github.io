...
Our raw source code first goes in to the lexer, then the parser, and then the virtual machine.
...
Add LALRPOP to your dependencies.
...
snailquote = "0.3.1" # We use this later
...
...
grammar(filename: &str, line_nr: usize); // Our parser will take a filename and a line number to construct good Locs

// This is our lexer, these are it's tokens
match {
    r#"[A-Z][A-Za-z0-9_]*"# => UpperCaseIdent,
    r#""([^"\\]|\\(.|\n))*""# => String,
    "(",
    ")",
    ",",
...
// ex. Print
function_name: Expr = {
    <l:@L> <a:UpperCaseIdent> => Expr { type_: ExprType::Variable(a.to_owned()), loc: Loc { filename: filename.to_owned(), line_nr, index: l } }
}
...
## VM
Next we implement a virtual machine.
