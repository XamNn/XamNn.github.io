...
We change the if statement into a match statement.
```rust
        LineType::EndScope => {
            vm.scope = 0;
...
```
When a **Return** statemnt is encountered we set **vm.scope_state** to **Return** and exit all loops. 
```rust
...
the **Ref**d variable to the parameter.
```rust
...
```rust
                value::Value::PrintBuiltin => {
                    for arg_expr in arg_exprs {
                        let param_value = expr_to_value(vm, arg_expr);
