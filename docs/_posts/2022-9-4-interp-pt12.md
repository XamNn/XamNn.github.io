...
```rust
pub struct VM {
    pub stack: HashMap<usize, value::Value>,
    ...
    match &parsed_line.type_ {
        ...
        LineType::EndScope => {
            ...
            match std::mem::replace(&mut vm.scope_state, ScopeState::Regular) {
                ScopeState::If(prev_cond) | ScopeState::Else(prev_cond) => {
                    if prev_cond {
...
### expr_to_value
Modify the **Call** case.
Memory used by local function variables is freed.
        ...
        ExprType::Call(fn_expr, param_exprs) => {
            ...
            match fn_value {
                value::Value::Function { params, lines } => {
                    ...
                    for i in 0..param_exprs.len().min(params.len()) {
                        if let ExprType::Ref(ref_expr) = &param_exprs[i].type_ {
                            ...
                            let value = expr_to_value(vm, &param_exprs[i]);
                            vm.stack.push(value);
                        }
                    }
                ...
                value::Value::PrintBuiltin => {
                    for param_expr in param_exprs {
                        let param_value = expr_to_value(vm, param_expr);
...
pub fn value_repr(val: value::Value) -> String {
    match val {
        ...
        value::Value::Function { params, .. } => format!("f({})", params.join(", ")),
    }
}
```
